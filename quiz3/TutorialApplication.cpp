...
void BasicTutorial_00::setMassSpringSystem()
{
	for(int j=0;j<m_NumCircles;j++){
		for(int i=0;i<m_NumSamples;i++){
			m_MassSpringSystem->addParticle(mSceneNode[i+j*m_NumSamples],mSmallSphereRadius,true);
			if(j>1){
				m_MassSpringSystem->addSpring(i+(j-2)*m_NumSamples,i+j*m_NumSamples);
			}
		}
	}
}
    ...
    )
{
    ...
    for (int j = 0; j < numCircles; ++j) {
        ...
        for (int i = 0; i < numSamples; ++i) {
			...
			float z = cos(a) * r;
            //
            mSceneNode[i+j*numSamples]->setPosition(x, h, z);
